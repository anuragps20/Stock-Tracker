...
class StockConsumer(AsyncWebsocketConsumer):
    ...
    def addToCeleryBeat(self, stockpicker):
        ...
        # if task is already there, than someone there to make a websocket connection instead
        # of creating new task we add that argument in our existing task, so celery will call
        # the api for those argument as well
        if len(task)>0:
            task = task.first()
